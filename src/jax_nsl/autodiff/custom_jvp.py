# File location: src/jax_nsl/autodiff/custom_jvp.py
...
@custom_jvp
def piecewise_linear_jvp(
    x: jnp.ndarray, breakpoints: jnp.ndarray, slopes: jnp.ndarray
) -> jnp.ndarray:
...
@custom_jvp
def saturated_activation_jvp(
    x: jnp.ndarray, lower_bound: float = -1.0, upper_bound: float = 1.0, smoothness: float = 1.0
) -> jnp.ndarray:
