...
def init_conv_params(key: Array, kernel_shape: Tuple[int, int], in_channels: int, out_channels: int,
                     init_type: str = "he", dtype: Any = jnp.float32) -> Dict[str, Array]:
...
def conv2d(x: Array, weights: Array, stride: Tuple[int, int] = (1, 1), padding: str = "SAME",
           dilation: Tuple[int, int] = (1, 1), feature_group_count: int = 1) -> Array:
    """Raw 2-D convolution, ``x: NHWC``, ``weights: OIHW``."""
    return lax.conv_general_dilated(x, weights, window_strides=stride, padding=padding,
                                    rhs_dilation=dilation, dimension_numbers=_DN,
                                    feature_group_count=feature_group_count)


def conv2d_layer(x: Array, params: Dict[str, Array], stride: Tuple[int, int] = (1, 1),
                 padding: str = "SAME", dilation: Tuple[int, int] = (1, 1),
                 activation: str = "relu") -> Array:
...
def conv_output_shape(input_hw: Tuple[int, int], kernel: Tuple[int, int], stride: Tuple[int, int],
...
def pooling_layer(x: Array, pool_type: str = "max", window_shape: Tuple[int, int] = (2, 2),
                  stride: Tuple[int, int] = (2, 2), padding: str = "VALID") -> Array:
...
def init_batch_norm_2d(channels: int, dtype: Any = jnp.float32) -> Tuple[Dict[str, Array], Dict[str, Array]]:
...
def batch_norm_2d(x: Array, params: Dict[str, Array], state: Dict[str, Array], training: bool = True,
                  momentum: float = 0.9, epsilon: float = 1e-5) -> Tuple[Array, Dict[str, Array]]:
    """Per-channel batch norm over ``(N, H, W)``; returns ``(out, new_state)``."""
    if training:
        mean = jnp.mean(x, axis=(0, 1, 2))
        var = jnp.var(x, axis=(0, 1, 2))
        new_state = {"running_mean": momentum * state["running_mean"] + (1 - momentum) * mean,
...
def depthwise_conv2d(x: Array, kernel: Array, stride: Tuple[int, int] = (1, 1),
...
def separable_conv2d(x: Array, depthwise_kernel: Array, pointwise_kernel: Array,
                     stride: Tuple[int, int] = (1, 1), padding: str = "SAME") -> Array:
...
def residual_block(x: Array, params1: Dict[str, Array], params2: Dict[str, Array],
                   shortcut_params: Optional[Dict[str, Array]] = None) -> Array:
...
def create_cnn(input_shape: Tuple[int, int, int], conv_channels: Sequence[int],
               dense_layers: Sequence[int], num_classes: int, kernel: Tuple[int, int] = (3, 3),
    ...
    for k, out_features in zip(keys[len(conv_channels):], [*dense_layers, num_classes]):
        dense_params.append({"weights": glorot_uniform_init(k, (in_features, out_features)),
